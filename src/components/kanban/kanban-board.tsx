...
export function KanbanBoard({
  ...
  const [editing, setEditing] = useState<{ columnId: string; card: KanbanCard; isNew: boolean } | null>(null);
  ...
  function addColumn() {
    ...
    const column: KanbanColumn = { id: Math.random().toString(36).slice(2, 10), name: name.trim(), cards: [] };
  ...
  function renameColumn(columnId: string) {
    ...
    persist({
      columns: board.columns.map((c) => (c.id === columnId ? { ...c, name: name.trim() } : c)),
    });
  }
  ...
  function deleteColumn(columnId: string) {
    const column = board.columns.find((c) => c.id === columnId);
    if (!column) return;
    if (column.cards.length > 0 && !window.confirm(`"${column.name}" tiene tarjetas. ¿Eliminarla igualmente?`)) {
      return;
    }
  ...
  function saveCard(nextCard: KanbanCard) {
    ...
    persist({
      columns: board.columns.map((column) => {
        ...
        return { ...column, cards: column.cards.map((c) => (c.id === nextCard.id ? nextCard : c)) };
      }),
    });
    setEditing(null);
  }
  ...
  function deleteCard() {
    ...
    persist({
      columns: board.columns.map((column) =>
        column.id === columnId ? { ...column, cards: column.cards.filter((c) => c.id !== card.id) } : column,
      ),
    });
    setEditing(null);
  }
  ...
  function moveCard(fromColumnId: string, cardId: string, toColumnId: string, beforeCardId: string | null) {
  ...
  function handleDropOnColumn(event: React.DragEvent, columnId: string) {
    ...
    if (!data) return;
    const { columnId: fromColumnId, cardId } = JSON.parse(data) as { columnId: string; cardId: string };
    moveCard(fromColumnId, cardId, columnId, null);
  }

  function handleDropOnCard(event: React.DragEvent, columnId: string, beforeCardId: string) {
    if (!isValidDrag(event)) return;
    event.preventDefault();
        ...
        {board.columns.map((column) => (
          <div
            ...
            className={`flex w-64 shrink-0 flex-col rounded-lg border border-border bg-surface ${
              dragOverColumn === column.id ? "ring-1 ring-inset ring-blue-400 dark:ring-blue-500" : ""
            }`}
          >
            ...
            <div className="flex flex-1 flex-col gap-2 overflow-y-auto px-2 pb-2">
              {column.cards.map((card) => (
                ...
                >
                  <p className="truncate text-sm text-zinc-800 dark:text-zinc-200">{card.title}</p>
